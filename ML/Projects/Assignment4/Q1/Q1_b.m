...
while itr~=0 & flag ==1
    ...
    itr=itr-1;
    mu_new(1,:)=(count_j1_x)/double(count_j1);
    mu_new(2,:)=(count_j2_x)/double(count_j2);
    mu_new(3,:)=(count_j3_x)/double(count_j3);
    mu_new(4,:)=(count_j4_x)/double(count_j4);
    
    for i=1:4
        if norm(mu(i,:)-mu_new(j,:))<.000001
            flag=0;
        end
    end
    mu=mu_new;
